...
class OrderedList extends Array {//order list 從小到大
    ...
    _binarySearch(element) {//寻找目前的值
        let low = 0;//从前面开始
        let high = this.length - 1;//整个排列的数值
        ...
        while (low <= high) {
            ...
            if (this[mid] === element) {
                ...
                low = mid + 1; // Search in the right half//右边开始算
            } else {
                high = mid - 1; // Search in the left half//左边开始算
            }
        }
    ...
    _findInsertIndex(element) {//增加新的值
        let low = 0;
